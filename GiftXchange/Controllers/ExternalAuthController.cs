...
namespace GiftXchange.Controllers
{
  ...
  public class ExternalAuthController : Controller
  {
    ...
    public ExternalAuthController(IOptions<FacebookAuthSettings> fbAuthSettingsAccessor,
        UserManager<GXUser> userManager,
    ...
    public async Task<IActionResult> Facebook([FromBody]FacebookAuthViewModel model)
    {
      ...
      if (user == null)
      {
        ...
        var result = await _userManager.CreateAsync(GXUser, Convert.ToBase64String(Guid.NewGuid().ToByteArray()).Substring(0, 8));

        if (!result.Succeeded) return new BadRequestObjectResult(Errors.AddErrorsToModelState(result, ModelState));

        //await _appDbContext.Customers.AddAsync(new Customer { IdentityId = GXUser.Id, Location = "", Locale = userInfo.Locale, Gender = userInfo.Gender });
